---
title: Choppa's Wiki
type: overview
public: true
created: 2026-04-21
updated: 2026-04-21
---

# Choppa's Wiki

> An open notebook. Thesis content, community mechanics, research, and drafts-in-public from [@ChoppaTheShark](https://x.com/ChoppaTheShark).

This site is a living knowledge base. Built in the [Karpathy pattern](https://karpathy.bearblog.dev/) — raw sources feed synthesis pages, which get refined over time. Pages link to each other freely. Some are polished, some are rough. That's the point.

## Start Here

**Pyth**
- [[pyth/index|The Pyth Community Maestro]] — what I do at Pyth Network, and the community operation behind it.
- [[pyth/philosophy|Community Philosophy]] — living machines, flywheels, and the Community Calculus.
- [[pyth/thesis|The Pyth Thesis]] — institutional insurgent, AI discoverability, agent economy.

**Reading**
- [[reading/index|Reading List]] — Community, Cults, Communication, Culture. What I've read, what I'm reading, and where it shows up in my work.

**Thesis**
- [[invisibility-tax]] — The hidden career cost of letting only your boss know you're good at your job.
- [[portfolio-as-proof]] — Why proof-of-thought is the new proof-of-work, and portfolios aren't just for designers anymore.

**Writings**
- [Why Communities Beat VCs](./writings/why-communities-beat-vcs/why-communities-beat-vcs) — The parent thesis. Flywheel, vibe economy, tokens as cultural infrastructure.
- [Creating a Community Cult(ure)](./writings/creating%20a%20community%20cult%28ure%29/Creating-a-community-cult%28ure%29) — Memetic community cults, political science applied to Web3.
- [Crabs in a Bucket](./writings/Crabs%20in%20a%20Bucket;%20a%20Crypto%20Seafood%20Boil/Crabs-in-a-Bucket-a-Crypto-Seafood-Boil) — Cultural immune systems, energy vampires, extraction defense.

**Sources**
- [[src-stop-being-invisible]] — Gearside on the invisibility tax and portfolio thesis.

## How This Works

- `wiki/` pages are curated thought — thesis, concepts, synthesis.
- `wiki/writings/` is the archive — published posts from 2024 onward.
- Links between pages use `[[wikilinks]]` — click them to navigate.
- Graph view (top right) shows how everything connects.

## Elsewhere

- Twitter: [@ChoppaTheShark](https://x.com/ChoppaTheShark)
- Substack (older writing): [choppa.substack.com](https://choppa.substack.com)

Built with [Quartz](https://quartz.jzhao.xyz).
